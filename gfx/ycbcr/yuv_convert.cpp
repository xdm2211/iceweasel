...
void ConvertYCbCrToRGB32(const uint8_t* y_buf, const uint8_t* u_buf,
  ...
  if (dx | dy) {
    ...
    auto convert = [&](int sx, int sy, int w, int h, uint8_t* dst) {
      if (w <= 0 || h <= 0) {
        return;
      }
      ...
      DebugOnly<int> err =
        fConvertYUVToARGB(py, y_pitch, pu, uv_pitch, pv, uv_pitch,
    ...
    if (dy) {
      ...
      if (dx) {
        ...
        convert(pic_x, pic_y, 1, 1, rgb_buf);
      }
      // Remaining columns of first row; source x pic_x+dx is even.
      convert(pic_x + dx, pic_y, pic_width - dx, 1, rgb_buf + dx * 4);
    }
    ...
    if (dx) {
      ...
      convert(pic_x, sy, 1, h, rgb_buf + dy * rgb_pitch);
    }
    // Main body: source (pic_x+dx, sy), both even-aligned.
    convert(pic_x + dx, sy, pic_width - dx, h,
